...
function createReleaseFixture(version: string): string {
	const directory = mkdtempSync(join(tmpdir(), "screwdriver-release-"));
	temporaryDirectories.push(directory);
	writeFileSync(join(directory, "package.json"), JSON.stringify({ version }));
	writeFileSync(join(directory, "manifest.json"), JSON.stringify({ version: "0.0.12", minAppVersion: "1.4.4" }));
	writeFileSync(join(directory, "versions.json"), JSON.stringify({ "0.0.12": "1.4.4" }));
	return directory;
}
...
describe("release version files", () => {
	it("records every release version even when the minimum app version is unchanged", () => {
		...
		expect(versions[version]).toBe("1.4.4");
	});

	it("prepares an optional Unreleased section", () => {
		const version = "0.0.13";
		const directory = createReleaseFixture(version);
		execFileSync(process.execPath, [versionBumpScript], {
			cwd: directory,
		...
		expect(readFileSync(join(directory, "updates.md"), "utf8")).toBe("## 0.0.13\n\n- Change\n");
	});
});
